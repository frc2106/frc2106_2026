 ...
 * 
 * <p>This subsystem implements a fully-featured swerve drive with:
 * - High-frequency odometry (250Hz on CAN FD, 100Hz otherwise)
 * - Vision integration for pose correction
 * - PathPlanner autonomous path following
 * - SysId characterization routines
 * - Real-time pose estimation with gyro fallback
 * - Comprehensive logging and telemetry
 * 
 * <p>The architecture separates concerns into:
 * 1. IO Layer (IO_ModuleBase, IO_GyroBase) - Hardware abstraction
 * 2. Module Layer (Module) - Individual swerve module control
 * 3. Drive Layer (Drive) - Coordination and odometry
 * 
 * <p>Odometry is processed on a separate thread (PhoenixOdometryThread) to achieve
 * high-frequency updates without being blocked by the main 20ms loop, enabling
 * accurate pose estimation even during aggressive maneuvers.
 */
public class Drive extends SubsystemBase {
    ...
    /**
     * Odometry update frequency (Hz) - depends on CAN FD capability.
     * CAN FD networks can handle 250Hz updates, while classic CAN is limited to 100Hz.
     * Higher frequency = more accurate pose estimation during dynamic movements.
     */
    static final double ODOMETRY_FREQUENCY =
            new CANBus(TunerConstants.DrivetrainConstants.CANBusName).isNetworkFD() ? 250.0 : 100.0;

    /**
     * Drive base radius (meters) - the distance from robot center to furthest module.
     * Calculated from module positions to ensure no module exceeds this radius during rotation.
     * Used for angular velocity calculations and constraint enforcement.
     */
     ...
     * 
     * <p>Sets up:
     * - Pose suppliers for starting pose and pose updates
     * - Chassis speeds supplier for velocity feedback
     * - Velocity consumer for motor commands
     * - Holonomic drive controller with PID constants
     * - Pathfinding for dynamic obstacle avoidance
     * - Logging callbacks for telemetry
     */
     ...
     * 
     * <p>Unlike the main 20ms loop, this method processes every odometry sample from the motor controllers,
     * providing much higher resolution for pose estimation. This is critical for accurate odometry during
     * fast movements and rotations.
     * 
     * <p>The method:
     * 1. Reads module positions and deltas from each sampled timestamp
     * 2. Updates gyro angle (real or calculated from kinematics)
     * 3. Applies the pose update with the exact timestamp
     */
     ...
     * 
     * <p>Transforms the camera positions from robot-relative to field-relative coordinates
     * using the current robot pose. This allows viewing camera frustums in 3D visualization.
     */
     ...
     * 
     * <p>Converts chassis speeds to module states, desaturates wheel speeds to prevent
     * commanding velocities beyond the robot's capability, and sends setpoints to modules.
     * 
     ...
     * 
     * <p>Modules are oriented toward the robot center, creating a mechanically locked
     * configuration. The formation automatically resets when nonzero velocity is commanded.
     */
     ...
     * 
     * <p>Quasistatic tests apply a slowly ramping voltage to characterize static friction
     * and velocity feedforward terms. The test starts with a 1-second zero-voltage period
     * to establish a baseline, then runs the SysId routine.
     */
     ...
     * 
     * <p>Dynamic tests apply a step voltage to characterize acceleration feedforward
     * and feedback gains. The test starts with a 1-second zero-voltage period
     * to establish a baseline, then runs the SysId routine.
     */
     ...
     * 
     * <p>Vision measurements are weighted by their standard deviations.
     * Lower std dev = more trust in the vision measurement.
     */
     ...
     * 
     * <p>Checks all 22 AprilTags on the field and returns the ID and distance
     * of the closest one. Used for auto-alignment and dynamic state selection.
     * 
