 ...
 * 
 * <p>Uses WPILib's AddressableLED and LEDPattern APIs to create dynamic, state-aware visualizations.
 * Patterns are selected based on:
 * - Current superstructure state (scoring level, intake mode, algae handling)
 * - Climb mode status (waiting/ready/go sequences)
 * - Robot pose error relative to auto starting position
...
public class SUB_Led extends SubsystemBase {
     ...
     * 
     * <p>Initializes all patterns, sets up the LED strip, and configures the default command.
     * The strip must be started before it will display any patterns.
     *
     ...
     * 
     * <p>When disabled, displays pose error if the robot has moved from its auto start position.
     * When enabled, displays climb mode pattern if active, otherwise shows state-based pattern.
     */
    @Override
     ...
     * 
     * <p>Maps each state to a distinct visual pattern:
     * - CLIMB: Green blink for upward movement
     * - CORAL_STATION: Intake pattern
     * - L1-L4: Gradient patterns with unique colors
     * - ALGAE: Teal flame effect
