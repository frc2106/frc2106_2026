...
/**
 * Vision subsystem that processes camera observations and provides pose estimates to the drive subsystem.
 * 
 * <p>Key responsibilities:
 * - Polls all camera IO implementations for new data
 * - Filters pose observations based on quality metrics
 * - Calculates appropriate standard deviations for each observation
 * - Provides pose estimates to the drive subsystem's pose estimator
 * - Logs comprehensive telemetry for debugging and replay
 * 
 * <p>Filtering criteria:
 * - Must have at least one tag
 * - Single-tag observations must have ambiguity below threshold
 * - Z-coordinate must be realistic (robot can't be in the air)
 * - Pose must be within field boundaries
 * - Only allowed tag IDs are processed (security measure)
 * 
 * <p>Standard deviations scale with tag distance and count:
 * - More tags = lower uncertainty
 * - Closer tags = lower uncertainty
 * - MEGATAG_2 gets higher uncertainty than PHOTONVISION
...
public class SUB_Vision extends SubsystemBase {
     ...
     * 
     * <p>For each camera:
     * 1. Updates inputs from IO layer
     * 2. Processes all pose observations
     * 3. Filters out low-quality observations
     * 4. Calculates standard deviations based on distance and tag count
     * 5. Sends accepted poses to the consumer (drive subsystem)
     * 6. Logs comprehensive telemetry for each camera and a summary
     */
    @Override
    public void periodic() {
        ...
        for (int cameraIndex = 0; cameraIndex < io.length; cameraIndex++) {
            ...
            for (var observation : inputs[cameraIndex].poseObservations) {
                ...
                // The uncertainty increases with the square of average tag distance and decreases with tag count
     ...
     * 
     * <p>Implemented by the drive subsystem to receive vision measurements.
     * The method signature matches WPILib's pose estimator requirements.
     */
    @FunctionalInterface
