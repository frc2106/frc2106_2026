 ...
 * 
 * <p>Interfaces with PhotonVision's PhotonCamera API to:
 * - Read multitag and single-tag pose estimation results
 * - Extract target observations for servoing
 * - Calculate robot poses from tag detections
 * - Track all detected tag IDs for debugging
 * 
 * <p>Handles both multitag results (more accurate, uses multiple tags) and single-tag results
 * (less accurate but works when only one tag is visible). All observations are timestamped
 * for correct fusion with the pose estimator.
 */
public class IO_VisionCamera implements IO_VisionBase {
    /** PhotonVision camera instance for this IO object. */
    protected final PhotonCamera camera;
    /** Transform from robot center to camera mounting position. Used to convert camera poses to robot poses. */
     ...
     * 
     * <p>Processes all unread results from the camera, extracting:
     * - Connection status
     * - Best target observation (for servoing)
     * - All pose observations (multitag and single-tag)
     * - All detected tag IDs
     * 
     * <p>Multitag results are more accurate as they use multiple tags simultaneously.
     * Single-tag results are used as fallback when only one tag is visible.
     * All poses are converted from camera-relative to robot-relative coordinates
     * using the robotToCamera transform.
     */
    @Override
